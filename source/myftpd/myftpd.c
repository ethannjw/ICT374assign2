...
void daemon_init(void)
{
    	...
    	sigemptyset(&act.sa_mask);       // set to not block other signals
    	act.sa_flags   = SA_NOCLDSTOP;   // set to not catch stopped children
...
int main(int argc, char *argv[])
{
	// variables
   	...
   	pid_t pid;
    	...
    	port = SERV_TCP_PORT; // set port number 
	...
	log_message(log_path, "Log File:\n%s\n", log_path);
    	...
    	bzero((char*)&ser_addr, sizeof(ser_addr));	// clear the addr first
    	...
    	// become a listening socket with backlog of 5
    	listen(sd, 5);
